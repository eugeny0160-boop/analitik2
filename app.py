...
# PORT больше не нужен в этом файле, его будет использовать Gunicorn
...
def save_post(title, content, url, pub_date):
    ...
    try:
        supabase.table("ingested_content_items").insert({
            ...
            "pub_date": pub_date.isoformat(), # pub_date уже должен быть timezone-aware
...
def generate_report():
    # Исправлено: используем timezone-aware время
    ...
    try:
        ...
        report_lines.append(f"Отчёт сформирован: {now.strftime('%d.%m.%Y %H:%M')} UTC") # Используем now
...
async def send_report_async():
    ...
    try:
        ...
        now = datetime.now(timezone.utc) # Используем timezone-aware время
        supabase.table("ingested_content_items") \
            .update({"is_analyzed": True}) \
            .gte("pub_date", (now - timedelta(days=1)).isoformat()) \
            .eq("is_analyzed", False) \
            .execute()
        return True
    except Exception as e:
    ...
    try:
        ...
        if update.channel_post:
            ...
            if update.channel_post.chat.id == SOURCE_CHANNEL_ID:
                ...
                # pub_date - время получения обновления, может быть ближе к реальному времени
                pub_date = datetime.now(timezone.utc)
                url = post.link or f"https://t.me/c/{post.chat.id}/{post.message_id}"
                save_post(post.text[:100], post.text, url, pub_date) # Передаём pub_date
            else:
...
# === ВАЖНО: Убираем вызов flask_app.run(...) ===
# def main():
